...
def fill_form_entries(entries, fill_algorithm, idx, village):
    ...
    if village not in [
        ...
        print(f"Cannot proceed with the request. Crosscheck the ward with form data, given village: {village}",)
        return
...
def get_form_submit_request(
    ...
    if fill_algorithm:
        entries = fill_form_entries(entries, fill_algorithm, idx, village)
    if not entries:
        return None
